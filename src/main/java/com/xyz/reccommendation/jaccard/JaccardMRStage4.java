...
public class JaccardMRStage4 {
	 ...
	 */
	public static class JaccardMap extends MapReduceBase implements
		...
		public void map(Object ikey, BSONObject ival,
			...
			if (ival.get("countJaccard") != null && ival.get("p1") != null) {
				TextDoublePair okey = new TextDoublePair();
				okey.set(ival.get("p1").toString(),
						Double.parseDouble(ival.get("countJaccard").toString()));
	 ...
	 */
	public static class JaccardReduce extends MapReduceBase implements
		...
		public void reduce(TextDoublePair ikey, Iterator<BSONObject> vlist,
			...
			while (vlist.hasNext() && count < 20) {
				...
				outputObj.put("countJaccard", val.get("countJaccard")
						.toString());
	...
	public static void main(String[] args) throws Exception {
		JobConf conf3 = new JobConf(JaccardMRStage4.class);
		conf3.setJobName("Map-Reduce for Jaccard 4");
		...
		conf3.setMapperClass(JaccardMap.class);
		conf3.setReducerClass(JaccardReduce.class);
...
//		MongoConfigUtil.setCreateInputSplits(conf3, false);
		...
		MongoConfigUtil.setOutputURI(conf3,
				"mongodb://54.251.196.236/products.out_stat_jaccard");
